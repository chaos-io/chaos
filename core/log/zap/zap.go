...
func init() {
	if err := zap.RegisterEncoder(encoders2.EncoderNameKV, encoders2.NewKVEncoder); err != nil {
		panic(err)
	}
	if err := zap.RegisterEncoder(encoders2.EncoderNameCli, encoders2.NewCliEncoder); err != nil {
		panic(err)
	}
	if err := zap.RegisterEncoder(encoders2.EncoderNameTSKV, encoders2.NewTSKVEncoder); err != nil {
		panic(err)
	}
}
...
var _ log2.Logger = &Logger{}
var _ log2.Structured = &Logger{}
var _ log2.Fmt = &Logger{}
var _ log2.LoggerWith = &Logger{}
var _ log2.LoggerAddCallerSkip = &Logger{}
...
func StandardConfig(encoding string, level log2.Level) zap.Config {
	return zap.Config{
		Level:            zap.NewAtomicLevelAt(ZapifyLevel(level)),
		Encoding:         encoding,
		OutputPaths:      []string{"stdout"},
...
func (l *Logger) Logger() log2.Logger {
	return l
}

// Fmt returns fmt logger
func (l *Logger) Fmt() log2.Fmt {
	return l
}

// Structured returns structured logger
func (l *Logger) Structured() log2.Structured {
	return l
}

// With returns logger that always adds provided key/value to every log entry
func (l *Logger) With(fields ...log2.Field) log2.Logger {
	return &Logger{
		L: l.L.With(zapifyFields(fields...)...),
	}
}

func (l *Logger) AddCallerSkip(skip int) log2.Logger {
	return &Logger{
...
func (l *Logger) Trace(msg string, fields ...log2.Field) {
...
func (l *Logger) Debug(msg string, fields ...log2.Field) {
	if ce := l.L.Check(zap.DebugLevel, msg); ce != nil {
		ce.Write(zapifyFields(fields...)...)
	}
}
...
func (l *Logger) Info(msg string, fields ...log2.Field) {
...
func (l *Logger) Warn(msg string, fields ...log2.Field) {
...
func (l *Logger) Error(msg string, fields ...log2.Field) {
...
func (l *Logger) Fatal(msg string, fields ...log2.Field) {
...
func (l *Logger) WithName(name string) log2.Logger {
	return &Logger{
		L: l.L.Named(name),
	}
}
