...
func convertLevel(level log2.Level) logrus.Level {
	switch level {
...
func SetLevel(level log2.Level) {
...
func (a *LogrusAdapter) Format(entry *logrus.Entry) ([]byte, error) {
	var name *string
	fields := make([]log2.Field, 0, len(entry.Data))
	for key, val := range entry.Data {
		...
		if !skip {
			fields = append(fields, log2.Any(key, val))
		}
	}

	var logger log2.Logger
	if a.adaptCallstack {
		logger = log2.AddCallerSkip(a.logger, getCallerDepth())
	} else {
		logger = a.logger
	}
...
func AdaptLogrus(logr *logrus.Logger, logger log2.Logger, level log2.Level, opts ...Option) {
...
func AdaptStandardLogger(logger log2.Logger, level log2.Level, opts ...Option) {
