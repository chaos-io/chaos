...
func (l *zapLogger) Log(entry Entry) {
	...
	case DebugLevel:
		if len(kv) == 0 {
			l.sugar.Debug(entry.Message)
			return
		}
		l.sugar.Debugw(entry.Message, kv...)
	case InfoLevel:
		if len(kv) == 0 {
			l.sugar.Info(entry.Message)
			return
		}
		l.sugar.Infow(entry.Message, kv...)
	case WarnLevel:
		if len(kv) == 0 {
			l.sugar.Warn(entry.Message)
			return
		}
		l.sugar.Warnw(entry.Message, kv...)
	case ErrorLevel:
		if len(kv) == 0 {
			l.sugar.Error(entry.Message)
			return
		}
		l.sugar.Errorw(entry.Message, kv...)
	case DPanicLevel:
		if len(kv) == 0 {
			l.sugar.DPanic(entry.Message)
			return
		}
		l.sugar.DPanicw(entry.Message, kv...)
	case PanicLevel:
		if len(kv) == 0 {
			l.sugar.Panic(entry.Message)
			return
		}
		l.sugar.Panicw(entry.Message, kv...)
	case FatalLevel:
		if len(kv) == 0 {
			l.sugar.Fatal(entry.Message)
			return
		}
		l.sugar.Fatalw(entry.Message, kv...)
	default:
		if len(kv) == 0 {
			l.sugar.Info(entry.Message)
			return
